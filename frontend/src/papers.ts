// Búsqueda de literatura en PubMed (modal). Puerto TS de js/papers.js, apuntando al
// nuevo endpoint FastAPI /api/papers.

import type { Patron } from './tipos.js';
import { manejadorAsync } from './async.js';

const PROXY_URL = '/api/papers';
const POR_PAGINA = 10;
...
let todosLosPapers: Paper[] = [];
let paginaActual = 0;
let consultaActual = '';

// PubMed sólo recibe texto en inglés; se traducen los nombres de patrón.
const TERMINOS_EN: Record<string, string> = {
  'Anemia': 'anemia',
...
const traducirPatron = (nombre: string): string => {
  for (const [es, en] of Object.entries(TERMINOS_EN)) {
    if (nombre.startsWith(es)) return en;
  }
  return nombre.replace(/[áéíóúñ]/g, (c) => ACENTOS[c] || c);
};

const construirQuery = (patrones: Patron[]): string => {
...
const buscarPapers = async (query: string): Promise<Paper[]> => {
  const respuesta = await fetch(`${PROXY_URL}?query=${encodeURIComponent(query)}`, { credentials: 'same-origin' });
  if (!respuesta.ok) {
    const detalle = await respuesta.json().catch(() => ({}));
    throw new Error(detalle.detail || detalle.error || `Error ${respuesta.status}`);
  }
  const datos = await respuesta.json();
  return datos.data || [];
};

const renderizarTarjetaPaper = (paper: Paper): HTMLElement => {
...
const renderizarPaginacion = (): void => {
...
const renderizarPaginaActual = (): void => {
...
const irAPagina = (numeroPagina: number): void => {
  const totalPaginas = Math.ceil(todosLosPapers.length / POR_PAGINA);
...
export const abrirModalPapers = async (patrones: Patron[]): Promise<void> => {
  ...
  try {
    ...
    mostrarError((error as Error).message || 'No se pudo conectar con PubMed. Intenta de nuevo más tarde.');
    console.error('Error buscando papers:', error);
  }
};

const cerrarModalPapers = (): void => {
  const modal = document.getElementById('modal-papers');
  const overlay = document.getElementById('modal-papers-overlay');
  if (!modal || !overlay) return;
...
export const inicializarModalPapers = (): void => {
  ...
  document.getElementById('papers-paginacion')?.addEventListener('click', (e) => {
    const btn = (e.target as Element).closest<HTMLButtonElement>('.papers-pag-btn');
    if (!btn || btn.disabled) return;
    irAPagina(parseInt(btn.dataset.pagina ?? '0', 10));
  });

  document.getElementById('papers-busqueda-form')?.addEventListener('submit', manejadorAsync('Búsqueda en PubMed', async (e: Event) => {
    e.preventDefault();
    const input = document.getElementById('papers-busqueda-input') as HTMLInputElement | null;
